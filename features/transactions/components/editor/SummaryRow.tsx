...
export function SummaryRow({
  ...
  const highlightAccent = isError ? themeColors.error : themeColors.primary;
  const chevronActive = isError ? themeColors.error : themeColors.primary;

  const backgroundStyle = useAnimatedStyle(() => ({
    opacity: Math.min(1, active.value + pressed.value * 0.22),
  }));
  ...
  return (
    <View className="overflow-hidden">
      <Animated.View
        pointerEvents="none"
        style={[StyleSheet.absoluteFillObject, { backgroundColor: highlightBg }, backgroundStyle]}
      />
      <Animated.View
        pointerEvents="none"
        style={[styles.accent, { backgroundColor: highlightAccent }, accentStyle]}
      />
