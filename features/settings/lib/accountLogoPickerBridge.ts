// The account logo picker hands its result back through an `onSelect` callback
// and receives the currently-selected id. The callback is non-serializable, so
...
export interface AccountLogoPickerSession {
  selectedLogoId: string | null;
  onSelect: (logoId: string | null) => void;
}

let pending: AccountLogoPickerSession | null = null;

/** Stash the hand-off right before navigating to the logo picker. */
export function setPendingAccountLogoPicker(session: AccountLogoPickerSession) {
  pending = session;
}

/** Reads and clears the pending hand-off (null after a cold state restore). */
export function consumePendingAccountLogoPicker(): AccountLogoPickerSession | null {
