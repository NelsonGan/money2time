...
import { Ban, Check, ChevronDown, ImagePlus, Search, Trash2, X } from 'lucide-react-native';
...
import {
  deleteCustomLogo,
  listCustomSubscriptionLogos,
  saveCustomSubscriptionLogo,
} from '~/services/userAssets';
import { cn } from '~/utils';
import { withColorAlpha } from '~/utils/color';

interface SubscriptionLogoPickerSheetProps {
  /** Dismisses the screen (pops the root stack back to the recurring editor). */
  onClose: () => void;
...
// Leads the (unfiltered) library grid so a rule that picked up a logo — from the
// picker or from the name-based suggestion — can always drop it again.
...
function LogoCell({
  ...
  return (
    ...
    >
      ...
      >
        <SubscriptionLogo logoId={meta.id} size={52} />
      </View>
      <Text
        variant="caption"
        numberOfLines={2}
...
export function SubscriptionLogoPickerSheet({
  onClose,
  selectedLogoId,
  onSelect,
}: SubscriptionLogoPickerSheetProps) {
  ...
  const refreshCustomLogos = useCallback(() => {
    setCustomLogos(listCustomSubscriptionLogos());
  }, []);

  const deviceDefaultCountry = useMemo(
    () => regionToSubscriptionCountry(getDeviceRegionCode()),
    [],
  );
  const activeCountry =
    settings.subscriptionLogoCountry &&
    SUBSCRIPTION_LOGO_COUNTRIES.some((c) => c.slug === settings.subscriptionLogoCountry)
      ? settings.subscriptionLogoCountry
      : (deviceDefaultCountry ?? DEFAULT_SUBSCRIPTION_COUNTRY);
  const activeCountryName =
    SUBSCRIPTION_LOGO_COUNTRIES.find((c) => c.slug === activeCountry)?.name ?? activeCountry;
  ...
  const results = useMemo<(SubscriptionLogoMeta | { id: typeof NONE_ITEM_ID })[]>(
    () =>
      isSearching
        ? searchSubscriptionLogos(query)
        : [{ id: NONE_ITEM_ID }, ...getSubscriptionLogosForCountry(activeCountry)],
    [activeCountry, isSearching, query],
  );

  const handleSelectCountry = (slug: string) => {
    void triggerHaptic('selection');
    updateSettings({ subscriptionLogoCountry: slug });
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      saveCustomSubscriptionLogo(result.assets[0].uri);
      refreshCustomLogos();
    } catch {
              ...
              return (
                <View style={styles.cell}>
                  ...
                  >
                    ...
                    >
                      <SubscriptionLogo logoId={logo.id} size={52} />
                    </View>
                  </Pressable>
                  <Pressable
          ...
          <View style={styles.flexOne}>
            <FlatList
              ...
              renderItem={({ item }) => {
                if (item.id === NONE_ITEM_ID) {
                  const selected = selectedLogoId == null;
                  return (
                    <Pressable
                      style={styles.cell}
                      ...
                      accessibilityLabel={I18n.t('recurring.logo.none')}
                    >
                      <View
                        style={[
                          styles.logoWrap,
                      ...
                      >
                        <View style={[styles.uploadTile, { borderColor: themeColors.border }]}>
                          <Ban size={20} color={themeColors.textMuted} />
                        </View>
                      </View>
                      <Text
                        variant="caption"
                ...
                const meta = item as SubscriptionLogoMeta;
                return (
                  <LogoCell
                    meta={meta}
            ...
            >
              ...
              >
                ...
                >
                  <Text className="text-[18px]">{getSubscriptionCountryFlag(activeCountry)}</Text>
                ...
                <TextInput
                  ...
                  placeholder={I18n.t('recurring.logo.search_placeholder')}
        ...
        <Pressable style={styles.countryBackdrop} onPress={() => setShowCountryModal(false)}>
          <Pressable onPress={(e) => e.stopPropagation()} style={styles.countrySheet}>
            ...
            >
              ...
              <FlatList
                data={SUBSCRIPTION_LOGO_COUNTRIES}
                  ...
                  return (
                    ...
                    >
                      <Text className="text-[20px]">
                        {getSubscriptionCountryFlag(country.slug)}
                      </Text>
                      <Text
                        variant="body"
                        numberOfLines={1}
