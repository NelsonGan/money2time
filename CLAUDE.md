...
| `transactionsTable`             | id, type (expense/income/transfer/balance_adjustment), amount, currency, **reportingCurrency, reportingAmount, fxRate** (frozen FX snapshot), **toAmount, accountAmount** (cross-currency), date, accountId, fromAccountId, toAccountId, categoryId, note, recurrence fields, sentiment, splitMethod (frozen split type: even/custom/items)                                                                                 |
| `transactionSplitsTable`        | id, transactionId, personName, amount, isSelf, isShared (portion of a shared item), paybackAccountId, paidAt, paidTransactionId, sortOrder, note (receipt line-item name)                                                                                                                                                                                                                                                   |
