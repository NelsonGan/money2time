...
- **Receipt scan**: Background OCR of a snapped/uploaded receipt, orchestrated by `context/ReceiptScanContext.tsx` (`useReceiptScans()` → `startScan`, `scanReceiptImage`, banner `jobs`). `startScan` gates the free-tier receipts limit, then opens the full-screen **custom camera** (`ScanReceiptCameraScreen` → lazy `components/ScanReceiptCamera`) via the `requestOpenScanCamera` bridge. The camera offers a shutter **and** an in-frame album button (bottom-right) so the user snaps or picks from their library without backing out; either path saves the image (`saveReceiptImage`) and calls `scanReceiptImage`, which enqueues a background job and hits the Worker (`services/receiptScan`). A single parsed draft becomes a tappable "ready to review" banner (opens a pre-filled editor); multiple drafts are added directly. **`expo-camera` is a native module** (config plugin in `app.json`), so `ScanReceiptCamera` is `React.lazy`-loaded behind an error boundary — adding/upgrading it needs a **dev-client/prebuild rebuild** (like MapLibre).
