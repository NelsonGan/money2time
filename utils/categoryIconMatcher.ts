...
// Suggests a bundled icon from a category or goal name. Patterns are tried in
// order; first match wins.
//
// The ids here are the suggestion's final answer, not an emoji that later gets
// mapped: routing a producer back through the legacy emoji table would
// reintroduce exactly the indirection this migration removed. Some emoji the
// old table distinguished collapse as a result (there is no pizza icon, so
// "Pizza" suggests the generic meal icon), which is the accepted cost.
export const CATEGORY_ICON_PATTERNS: { icon: string; pattern: RegExp }[] = [
  {
    icon: 'house',
    pattern:
...
if (process.env.NODE_ENV !== 'production') {
  for (const { icon } of CATEGORY_ICON_PATTERNS) {
    if (!CATEGORY_ICON_SOURCES[icon]) {
      console.warn(`categoryIconMatcher: ${icon} is not a bundled category icon`);
    }
  }
}

/** Bundled icon id matching the name, or null when nothing matches. */
export function suggestCategoryIcon(name: string): string | null {
  const trimmed = name?.trim() ?? '';
  if (!trimmed) return null;
  for (const { icon, pattern } of CATEGORY_ICON_PATTERNS) {
