...
`register` is an upsert keyed by the push token, so the app calls it on start
**and** on every foreground: that one call covers first registration, repair
after being offline, and a token ActivityKit rotated mid-session.
