...
This Worker accepts a PDF and an account from the app and extracts transactions with the same primary and backup OpenRouter models as receipt scanning. The Pro gate is temporarily disabled for simulator testing. It reserves one of 100 monthly scans before calling the model and releases the reservation if the call fails. A successful response contains transactions for the app's review screen; the Worker does not save the PDF.
...
The Worker shares the receipt scanner's D1 database. Apply `../../d1/statement-import/schema.sql` before deploying. Configure these Worker secrets with the same values used by the receipt scanner:
...
Run `npm ci`, `npm run typecheck`, and `npm test` here. The integration test uses sample and password-locked PDFs with a local model stub. `npx wrangler deploy --dry-run` verifies the Cloudflare bundle. A live scan is still required after the OpenRouter secret is installed.
