...
Run `npm ci`, `npm run typecheck`, and `npm test` here. The integration test uses sample and password-locked PDFs with a local model stub and checks both preview access and production Pro gating. `npx wrangler deploy --dry-run` verifies the Cloudflare bundle. A live scan is still required after the OpenRouter secret is installed.
